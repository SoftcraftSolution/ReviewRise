...
(async () => {
  const patches = [
    ...
    `ALTER TABLE reviews ADD COLUMN IF NOT EXISTS trust_issued BOOLEAN DEFAULT false`,
    `ALTER TABLE reviews ADD COLUMN IF NOT EXISTS google_verified BOOLEAN DEFAULT false`,
    
    `ALTER TABLE ad_views ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()`,
    `ALTER TABLE coupons ADD COLUMN IF NOT EXISTS source VARCHAR(50)`,
    `ALTER TABLE brands ADD COLUMN IF NOT EXISTS gmb_connected BOOLEAN DEFAULT false`,
    `ALTER TABLE brands ADD COLUMN IF NOT EXISTS gmb_connected_at TIMESTAMPTZ`,
    `ALTER TABLE brands ADD COLUMN IF NOT EXISTS gmb_token_expiry TIMESTAMPTZ`,
    `ALTER TABLE reviews ADD COLUMN IF NOT EXISTS gmb_review_id TEXT`,
    `ALTER TABLE reviews ADD COLUMN IF NOT EXISTS google_verified BOOLEAN DEFAULT false`,
    `CREATE UNIQUE INDEX IF NOT EXISTS reviews_gmb_review_id_idx ON reviews(gmb_review_id) WHERE gmb_review_id IS NOT NULL`,
    `ALTER TABLE banners ADD COLUMN IF NOT EXISTS subtitle TEXT`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS points_redeemed INTEGER DEFAULT 0`,
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n⭐ ReviewRise v12.1 running on port ${PORT}\n`);
  // Start background verification engine
  require('./verifyEngine').startEngine();
});
module.exports = app;
