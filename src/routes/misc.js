...
router.put('/banners/:id', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const { title, image_url, brand_id, active } = req.body;
    const r = await query(
      `UPDATE banners SET title=COALESCE($1,title),image_url=COALESCE($2,image_url),brand_id=COALESCE($3,brand_id),active=COALESCE($4,active),updated_at=NOW() WHERE id=$5 RETURNING *`,
      [title, image_url||null, brand_id, active, req.params.id]
    );
    res.json(r.rows[0]);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/ads/:id', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const { title, description, image_url, brand_id } = req.body;
    const r = await query(
      `UPDATE ads SET title=COALESCE($1,title),description=COALESCE($2,description),image_url=COALESCE($3,image_url),brand_id=COALESCE($4,brand_id),updated_at=NOW() WHERE id=$5 RETURNING *`,
      [title, description, image_url||null, brand_id, req.params.id]
    );
    res.json(r.rows[0]);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
