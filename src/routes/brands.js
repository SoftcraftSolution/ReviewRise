...
// PUT /api/brands/:id — update brand + optionally reset owner password
router.put('/:id', authenticate, async (req, res) => {
  try {
    const {
      name, category, emoji, location, plan, google_place_id,
      reward_offer, reward_min_order, coupon_validity_days, active,
      owner_name, owner_email, owner_phone, new_password
    } = req.body;

    const r = await query(`
      UPDATE brands SET
        name=COALESCE($1,name), category=COALESCE($2,category),
        emoji=COALESCE($3,emoji), location=COALESCE($4,location),
        plan=COALESCE($5,plan), google_place_id=COALESCE($6,google_place_id),
        reward_offer=COALESCE($7,reward_offer),
        reward_min_order=COALESCE($8,reward_min_order),
        coupon_validity_days=COALESCE($9,coupon_validity_days),
        active=COALESCE($10,active),
        owner_name=COALESCE($11,owner_name),
        owner_email=COALESCE($12,owner_email),
        owner_phone=COALESCE($13,owner_phone),
        updated_at=NOW()
      WHERE id=$14 RETURNING *
    `, [name,category,emoji,location,plan,google_place_id||null,
        reward_offer,reward_min_order?parseInt(reward_min_order):null,
        coupon_validity_days?parseInt(coupon_validity_days):null,
        active!=null?active:null,
        owner_name,owner_email?owner_email.trim().toLowerCase():null,
        owner_phone,req.params.id]);
    ...
    if (brand.owner_id) {
      if (new_password && new_password.trim().length >= 6) {
        ...
        await query(`UPDATE users SET password_hash=$1, updated_at=NOW() WHERE id=$2`, [hash, brand.owner_id]);
      }
      if (owner_email) {
        await query(`UPDATE users SET email=LOWER($1), updated_at=NOW() WHERE id=$2`, [owner_email.trim(), brand.owner_id]);
      }
      if (owner_name) {
        await query(`UPDATE users SET name=$1, updated_at=NOW() WHERE id=$2`, [owner_name, brand.owner_id]);
      }
    }
...
router.get('/:id/stats', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [cStats, rStats] = await Promise.all([
      query(`SELECT status, COUNT(*) as count FROM coupons WHERE brand_id=$1 GROUP BY status`, [id]),
      query(`SELECT COUNT(*) as total, AVG(stars)::numeric(3,1) as avg_stars FROM reviews WHERE brand_id=$1`, [id]),
    ]);
    const coupons = { active:0, redeemed:0, expired:0 };
    cStats.rows.forEach(r => { coupons[r.status] = parseInt(r.count); });
    res.json({ coupons, reviews: rStats.rows[0] || { total:0, avg_stars:0 } });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
